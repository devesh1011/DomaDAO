...
export class FractionPoolService {
  ...
  static getInstance(poolAddress: string): FractionPoolService {
    if (!FractionPoolService.instances.has(poolAddress)) {
      FractionPoolService.instances.set(poolAddress, new FractionPoolService(poolAddress))
    }
  ...
  async initialize(): Promise<void> {
    ...
    this.contract = new Contract(this.poolAddress, FractionPoolABI.abi, this.signer)
  }
  ...
  async initializeReadOnly(): Promise<void> {
    ...
    this.provider = new BrowserProvider(window.ethereum)
    this.contract = new Contract(this.poolAddress, FractionPoolABI.abi, this.provider)
  }
  ...
  private ensureInitialized(): Contract {
    if (!this.contract) {
      throw new Error('FractionPoolService not initialized. Call initialize() first.')
    }
  ...
  async getPoolInfo(): Promise<PoolInfo> {
    ...
    // Get metadata struct and individual fields
    const [
      metadata,
      totalRaised,
      state,
    ...
    return {
      ...
      state: Number(state) as PoolState,
      contributorCount,
      candidateCount,
  ...
  async contribute(amount: string): Promise<TransactionResponse> {
    ...
    const tx = await contract.contribute(amountRaw)
    return tx
  }
   ...
   */
  async getContribution(userAddress: string): Promise<{ amount: string; amountRaw: bigint }> {
    const contract = this.ensureInitialized()
  ...
  async getContributors(): Promise<Contributor[]> {
    const contract = this.ensureInitialized()
    const count = await contract.contributorCount()
    
    const contributors: Contributor[] = []
    for (let i = 0; i < Number(count); i++) {
      const address = await contract.contributors(i)
  ...
  async proposeCandidate(
    ...
    const tx = await contract.proposeCandidate(domainName, nftContract, tokenIdBigInt)
    return tx
  }
  ...
  async getCandidate(index: number): Promise<DomainCandidate> {
    ...
    return {
      ...
      nftContract: candidate.nftContract,
  ...
  /**
   * Vote for a domain candidate
   */
  async voteForCandidate(candidateIndex: number): Promise<TransactionResponse> {
    const contract = this.ensureInitialized()
   ...
   */
  async recordFractionalization(fractionTokenAddress: string): Promise<TransactionResponse> {
    const contract = this.ensureInitialized()
   ...
   */
  onContribution(callback: (contributor: string, amount: bigint) => void): () => void {
    const contract = this.ensureInitialized()
   ...
   */
  onCandidateProposed(
    callback: (proposer: string, candidateIndex: number, domainName: string) => void
  ): () => void {
    const contract = this.ensureInitialized()
 ...
 */
export async function getFractionPoolService(poolAddress: string): Promise<FractionPoolService> {
 ...
 */
export async function getReadOnlyFractionPoolService(poolAddress: string): Promise<FractionPoolService> {
  const service = FractionPoolService.getInstance(poolAddress)
