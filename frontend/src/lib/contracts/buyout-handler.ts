...
export class BuyoutHandlerService {
  ...
  async initialize(): Promise<void> {
    ...
    this.contract = new Contract(contractAddress, BuyoutHandlerABI.abi, this.signer)
  }
  ...
  private ensureInitialized(): Contract {
    if (!this.contract) {
      throw new Error('BuyoutHandlerService not initialized. Call initialize() first.')
    }
  ...
  async getPoolOffers(poolAddress: string): Promise<BuyoutOffer[]> {
    const contract = this.ensureInitialized()
    const offerIds = await contract.poolOffers(poolAddress)
  ...
  async proposeBuyout(
    ...
    const contract = this.ensureInitialized()
    const tx = await contract.proposeBuyout(poolAddress, offerAmount, expirationTime)
    return tx
  }
   ...
   */
  onBuyoutProposed(
    callback: (offerId: bigint, buyer: string, poolAddress: string, amount: bigint) => void
  ): () => void {
    const contract = this.ensureInitialized()
   ...
   */
  onBuyoutVoteCast(
    callback: (offerId: bigint, voter: string, accept: boolean, weight: bigint) => void
  ): () => void {
    const contract = this.ensureInitialized()
    
    const filter = contract.filters.BuyoutVoteCast()
    const listener = (offerId: bigint, voter: string, accept: boolean, weight: bigint) => {
   ...
   */
  onBuyoutAccepted(callback: (offerId: bigint, totalVotes: bigint) => void): () => void {
    const contract = this.ensureInitialized()
   ...
   */
  static calculateVotePercentage(acceptVotes: bigint, rejectVotes: bigint): number {
