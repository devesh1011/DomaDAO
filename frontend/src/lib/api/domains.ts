...
export const domainsApi = {
   ...
   */
  async searchDomains(params: SearchParams & { includePricing?: boolean }): Promise<PaginatedResponse<Domain>> {
    ...
    if (params.tld && params.tld !== "All") {
      ...
      const tldValue = params.tld.startsWith('.') ? params.tld.slice(1) : params.tld
    ...
    const response = await apiClient.get<any>('/api/domains/search', backendParams)
    
    // Transform DOMA API response format to our expected format
    if (response && response.__typename === 'PaginatedNamesResponse' && Array.isArray(response.items)) {
      ...
      const transformedData = {
        data: items.map((item: any) => {
          ...
          return {
            ...
            owner: item.tokens?.[0]?.ownerAddress || item.claimedBy || 'Unknown',
      ...
      if (params.includePricing) {
        const pricingPromises = transformedData.data.map(async (domain: Domain) => {
          try {
            const bestOffer = await this.getDomainBestOffer(`${domain.name}${domain.tld}`)
            return { ...domain, bestOffer }
          } catch (error) {
            console.error(`Error fetching pricing for ${domain.name}${domain.tld}:`, error)
  ...
  async getDomain(name: string): Promise<Domain> {
    try {
      ...
      if (response && response.name) {
        return {
          name: response.name,
          tld: response.name.includes('.') ? '.' + response.name.split('.').pop() : '',
          expiresAt: response.expiresAt,
          tokenizedAt: response.tokenizedAt,
          registrar: response.registrar?.name || 'Unknown',
          owner: response.tokens?.[0]?.ownerAddress || response.claimedBy || 'Unknown',
  ...
  async getDomainBestOffer(name: string): Promise<DomainOffer | null> {
    try {
      ...
      const bestOffer = offers.data.reduce((best, current) => {
        const bestPrice = parseFloat(best.price) / Math.pow(10, best.currency.decimals)
        const currentPrice = parseFloat(current.price) / Math.pow(10, current.currency.decimals)
