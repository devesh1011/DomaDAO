...
import { poolsApi, domainsApi } from '@/lib/api'
...
function useApiData<T>(
  ...
  const refetch = useCallback(async () => {
    try {
      ...
      const errorDetails = err?.response?.data?.error || err?.response?.statusText || ''
...
export function useDomainSearch(params: {
  ...
  return useApiData(
    ...
    [params?.query, params?.tld, params?.page, params?.limit, params?.includePricing]
  )
}
